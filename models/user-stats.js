...
async function getUserStats(searchType, startTime, endTime, limit, offset)
{
    ...
    if(searchType.toLowerCase() == "top" || searchType.toLowerCase() == "buyer" || searchType.toLowerCase() == "seller" || searchType.toLowerCase() == "royalties")
    {
        ...
        {
            resultArray = await GetTopBuyerStats(limit, offset, startTime, endTime)
           
        }
        if(searchType.toLowerCase() == "seller")
        {
            resultArray = await GetTopSellerStats(limit, offset, startTime, endTime)
          
        }
        if(searchType.toLowerCase() == "royalties")
        {
            resultArray = await GetTopRoyaltyStats(limit, offset, startTime, endTime)
...
function UserStats(user_address_b16, lifetime_sales_usd, lifetime_quantity, 
                   WZIL_volume, GZIL_volume, XSGD_volume, zWBTC_volume, zWETH_volume, zUSDT_volume, DUCK_volume)
{
    return {
        user_address_b16,
...
async function GetTopBuyerStats(limit_rows, offset_rows, startTime, endTime)
{
    ...
    {
        const row = UserStats(result.rows[res].buyer_address, result.rows[res].lifetime_sales_usd, result.rows[res].lifetime_quantity_sold, 
                        result.rows[res].wzil_volume, result.rows[res].gzil_volume, result.rows[res].xsgd_volume, result.rows[res].zwbtc_volume, result.rows[res].zweth_volume, result.rows[res].zusdt_volume, result.rows[res].duck_volume)
...
async function GetTopSellerStats(limit_rows, offset_rows, startTime, endTime)
{
    ...
    for(var res in result.rows)
    {
        const row = UserStats(result.rows[res].seller_address, result.rows[res].lifetime_sales_usd, result.rows[res].lifetime_quantity_sold, 
            result.rows[res].wzil_volume, result.rows[res].gzil_volume, result.rows[res].xsgd_volume, result.rows[res].zwbtc_volume, result.rows[res].zweth_volume, result.rows[res].zusdt_volume, result.rows[res].duck_volume)
        resultArray.push(row)
    }
    return resultArray
...
async function GetTopRoyaltyStats(limit_rows, offset_rows, startTime, endTime)
{
    ...
    for(var res in result.rows)
    {
        const row = UserStats(result.rows[res].royalty_address, result.rows[res].lifetime_sales_usd, result.rows[res].lifetime_quantity_royalty, 
            result.rows[res].wzil_volume, result.rows[res].gzil_volume, result.rows[res].xsgd_volume, result.rows[res].zwbtc_volume, result.rows[res].zweth_volume, result.rows[res].zusdt_volume, result.rows[res].duck_volume)
        resultArray.push(row)
    }
    return resultArray
